...
def new_merge_graphs(G_analyzed, persistent_graph):
    ...
    for node in overlapping_nodes:
        ...
        if target_community is not None:
            ...
            for n, d in G_analyzed.nodes(data=True):
                if (
                    d.get("status") == "suspicious"
                    and d["community"] == G_analyzed_community
                ):
                    d["community"] = target_community
                    print(
    ...
    for community in set(original_G_analyzed_communities.values()):
        if community not in merged_communities:
            nodes_in_community = [
                ...
                if comm == community
                and G_analyzed.nodes[node].get("status") == "suspicious"
            ]
